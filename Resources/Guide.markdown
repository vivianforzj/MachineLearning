对于机器学习，一直处于知道几个流行算法的“名字”的阶段。现在因为工作需要，终于下定决心入坑了。主要追随一大牛的网站（http://MachineLearningMastery.com）。自己偶尔也要做点小笔记。

大牛做了如下推荐：

## Books

### 初学者

入门级别的书，可对机器学习有一个初步的了解：

- Predictive Analytics: The Power to Predict Who Will Click, Buy, Lie, or Die

- Data Smart: Using Data Science to Transform Information into Insight

- Data Science for Business: What You Need to Know about Data Mining and Data-Analytic Thingking

### 实践性书籍

如果你跟我一样，也是个做工程的，肯定会想要实践一下：

- Applied Predictive Modeling

- Data Science from Scratch: First Principles with Python

- An Introduction to Statistical Learning: With Applications in R

- Data Mining: Practical Machine Learning Tools and Techniques

### 理论书籍

更为专业的理论，需要像上大学那样看课本了：

- The Elements of Statistical Learning: Data Mining, Inference, and Prediction

- Machine Learning: The Art and Science of Algorithms that Make Sense of Data

- Machine Learning: A Probabilistic Perspective

## 社区

### Stack Exchange

- [Cross Validated](http://stats.stackexchange.com/)

- [Quantitative Finance](http://quant.stackexchange.com/)

- [Programmers](http://programmers.stackexchange.com/)

- [Stack Overflow](http://stackoverflow.com/)

### Reddit

- [Machine Learning](https://www.reddit.com/r/machinelearning)

- [Statistics](https://www.reddit.com/r/statistics)

- [Data Science](https://www.reddit.com/r/datascience)

- [Big Data](https://www.reddit.com/r/bigdata)

### Quora

- [Machine Learning](https://www.quora.com/topic/Machine-Learning)

- [Statistics](https://www.quora.com/topic/Statistics-academic-discipline]

- [Data Mining](https://www.quora.com/topic/Data-Mining)

- [Data Science](https://www.quora.com/topic/Data-Science)

## 软件

### 初学者

WEKA，有界面，也有JAVA API。学习见http://www.cs.waikato.ac.nz/ml/weka/ 。

### 中级用户

Python scikit-learn，基于SciPy(NumPy, Matplotlib and Pandas). 详见http://scikit-learn.org/stable/。

### 高级用户

R，基于caret包。详见http://caret.r-forge.r-project.org/。